  ...
  <q-page padding>
                    ...
                    <q-card flat>
                      <q-card-section>
                        <div class="row">
                          ...
                          <div v-if="props.row.type.type === 'SCHEDULE_CHANGE'">
    ...
    <q-dialog v-model="filtersDialog">
      <q-card class="dialog-width-default q-pa-sm">
        ...
        <q-card-section class="q-px-md q-pb-md q-mx-sm q-mt-lg q-ma-none">
          <q-select v-model="filters.typeId" options-dense :options="scheduleTypes" dense
    ...
    <q-dialog v-model="createDialog">
      <q-card class="dialog-width-default q-pa-sm">
        ...
        <q-card-section class="q-px-md q-pb-md q-mx-sm q-mt-lg q-ma-none">
          <q-select v-model="item.type.id" options-dense :options="scheduleChangeTypes" dense
                    map-options emit-value option-label="title" option-value="id"
                    label="Тип"/>
        </q-card-section>
        <q-card-section>
        ...
        <q-card-section class="row q-mx-sm q-pb-md justify-between">
          ...
          <q-btn v-if="enableTelegramSending"
                 @click="sendTelegramMessage(image, item.description, null, true)"
    ...
    <q-dialog v-model="editDialog">
      <q-card class="dialog-width-default q-pa-sm">
        ...
        <q-card-section class="row q-mx-sm q-pb-md justify-between">
          ...
          <q-btn v-if="enableTelegramSending"
                 class="filter-button"
                 @click="sendTelegramMessage(image, item.description, item.file, true)"
...
import ScheduleTypeService from "src/services/schedule/scheduleTypeService";
...
export default defineComponent({
  ...
  setup() {
    ...
    const isValidItem = (item, file, action) => {
      const firstCheck = item.type.id;
      if (action === 'CREATE') {
        return firstCheck && (file || item.description);
      }
      if (action === 'UPDATE') {
        return firstCheck && ((item.file && item.file.id) || file || item.description);
      }
      return false;
    }
    ...
    const search = async (pageNumber, rowsPerPage, sortBy, descending, filters) => {
      const data = await ScheduleService.getPage({
    ...
    const deleteItem = async (item) => {
      try {
        await ScheduleService.deleteById(item.id);
        updateTable();
        $q.notify({
          type: 'positive',
    ...
    const editItem = async (data, image, sendMessage) => {
      try {
        await ScheduleService.update(data, image);
        updateTable();
        $q.notify({
          type: 'positive',
    ...
    const createItem = async (data, image, sendMessage) => {
      try {
        await ScheduleService.create(data, image);
        updateTable();
        $q.notify({
          type: 'positive',
    ...
    onMounted(async () => {
      scheduleTypes.value = await ScheduleTypeService.getAll();
      scheduleChangeTypes.value = scheduleTypes.value.filter(type => type.type === 'SCHEDULE_CHANGE');
      tableRef.value.requestServerInteraction();
    });
