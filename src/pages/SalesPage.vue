...
import SaleService from "src/services/sale/saleService";
...
export default defineComponent({
  ...
  setup() {
    ...
    const search = async (pageNumber, rowsPerPage, sortBy, descending, filters) => {
      const data = await SaleService.getPage({
    ...
    const deleteItem = async (item) => {
      try {
        await SaleService.deleteById(item.id);
        updateTable();
        $q.notify({
          type: 'positive',
    ...
    const editItem = async (data, image, sendMessage) => {
      try {
        await SaleService.update(data, image);
        updateTable();
        $q.notify({
          type: 'positive',
    ...
    const createItem = async (data, image, sendMessage) => {
      try {
        await SaleService.create(data, image);
        updateTable();
        $q.notify({
          type: 'positive',
